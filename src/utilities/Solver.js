...
import MoveSorter from '@/utilities/MoveSorter'
...
class Solver {
  constructor(width) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      this.columnExpOrderRev[width - 1 - x] =
        Math.ceil(width / 2 + ((1 - 2 * (x % 2)) * (x + 1)) / 2) - 1
    }
  }
  ...
  negamax(pos, alpha, beta) {
    ...
    if (pos.canWinNext()) return (pos.width * pos.height + 1 - pos.nbMoves) / 2

    let nextPossibleMove = pos.possibleNonLosingMoves()
    // Check if there's no possible *non* losing moves
    if (nextPossibleMove === 0n) return -(pos.width * pos.height - pos.nbMoves) / 2
    ...
    for (let i = pos.width; i--; ) {
      ...
      if (move) moves.add(move, pos.moveScore(move))
    }
